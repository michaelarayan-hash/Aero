...
async def main():
    ...
    try:
        await asyncio.wait_for(fly_square(drone=drone), timeout=20)
    except asyncio.TimeoutError as e:
...
async def fly_square(drone: System):
    ...
    while True:
        for name, NED in ned_map.items():
            ...
            print(NED)
            await drone.offboard.set_velocity_body(VelocityBodyYawspeed(NED[0], NED[1], NED[2], NED[3]))
            await asyncio.sleep(NED[4])
